...
6. **Строгая анонимность инфраструктуры и запрет деанонимизации:**
   * Категорически запрещено указывать реальные IP-адреса боевых серверов (включая Senko) в открытом виде в коде, разметке Astro, скриптах деплоя, документации или примерах команд. Использовать исключительно переменную `$SENKO_HOST` / секрет `${{ secrets.SENKO_HOST }}`.
   * Запрещено упоминать стороннее ПО хоста (VPN, прокси, личные сервисы) в документации и статьях портала. Любые сетевые особенности формулировать нейтрально (например, «порты 80/443 используются системными службами хоста»).
