...
## Фаза 20: Поисковая оптимизация (SEO), устранение ошибок сканирования и расширение базы знаний (Веб-портал)
...
## Фаза 21: Мета-летопись, типографика Кнута — Пласса и интерактивная веб-публикация хроники проекта (Веб-портал)
- [x] Интерактивный веб-раздел летописи проекта (`apps/website`):
  - [x] Роутинг и коллекции контента `chronicle` (Astro 5 Content Layer API, поддержка мультиязычности `en`/`ru`).
  - [x] Главный таймлайн `TimelineTrack.astro` с вертикальной световой линией и чередованием карточек.
  - [x] 8 полноразмерных глав (Акты I–VIII на русском и английском языках).
  - [x] Интеграция подлинных стенограмм разработки из логов сессий под сворачиваемые спойлеры `<DialogueSnippet />` с начальным закрытым состоянием.
- [x] Типографика Кнута — Пласса и укрощение выключки по ширине:
  - [x] Интеграция стандартов микротипографики на базе собственного MCP-сервера автора (`kobaltgit/mcp-typography-server` / Justif).
  - [x] Внедрение полноценной выключки `text-align: justify; text-justify: inter-word; hyphens: auto;` без визуальных растяжений и «белых рек».
  - [x] Атрибуция сервера типографики в подвале статей летописи.
- [x] Адаптивное оглавление `ChapterTOC.astro`:
  - [x] Встроенный компактный блок в начале статьи на мобильных устройствах.
  - [x] Плавный выезд вправо (`translate-x-0`) в парящую боковую панель при прокрутке страницы на десктопе.
  - [x] Активный трекинг прочитанных разделов (Scrollspy).
- [x] SEO-оптимизация, микроразметка и RSS-фиды хроники:
  - [x] Разметка Schema.org `TechArticle` для глав и `Blog` + `BreadcrumbList` для каталога.
  - [x] Расширенные теги Open Graph (`article:published_time`, `article:author`, `article:tag`) и Twitter Cards (`summary_large_image`).
  - [x] Нативные статические RSS 2.0 фиды для обеих языковых версий (`/chronicle/rss.xml` и `/ru/chronicle/rss.xml`).
  - [x] Полная индексация всех 8 глав и таймлайнов в поисковом индексе Pagefind и карте сайта `sitemap-0.xml`.
- [x] Интерактивный симулятор безопасности хост-ключей (`RandomartVisualizer.astro`):
  - [x] Сценарий 1: Первое подключение к новому серверу (TOFU диалог подтверждения ключа и сохранение в Vault).
  - [x] Сценарий 2: Штатный фоновый вход за 1.2 мс без лишних окон и вопросов (100% автопроверка).
  - [x] Сценарий 3: Атака перехвата (MitM) — мгновенный разрыв Fail-Closed и наглядное сравнение оригинального и поддельного узоров.
- [x] Синхронизация репозитория:
  - [x] Запись 46 в `docs/CHRONICLE.md` и Entry 46 в `docs/CHRONICLE.en.md`.
