...
- [x] Двухпанельный SFTP-менеджер файлов (Termius-like):
  ...
  - [ ] Исправление дефектов SFTP (BUG-025, BUG-026, BUG-027):
    - [ ] Рекурсивное скачивание и выгрузка папок с сохранением структуры каталогов (`BUG-025`).
    - [ ] Множественное выделение файлов и папок (`Shift`, `Ctrl/Cmd`, `Ctrl+A`) и пакетные операции (`BUG-026`).
    - [ ] Диалог подтверждения при совпадении имён файлов: Overwrite / Skip / Rename / Apply to all (`BUG-027`).
  - [ ] Развитие файлового менеджера SFTP Pro (`IDEA-027`):
    - [ ] Двусторонний Drag & Drop между локальной и удаленной панелями, а также перетаскивание файлов из проводника ОС.
    - [ ] Горячие клавиши навигации (`Up/Down`, `Enter`, `Backspace`, `Alt+Up`, `Delete`, `F2`) и буфер обмена (`Ctrl+C`, `Ctrl+X`, `Ctrl+V`).
    - [ ] Встроенный легковесный редактор текстовых файлов/конфигов и быстрый просмотр по Space (Quick Look).
    - [ ] Докачка файлов (Resume) и расширенное управление очередью передач (пауза, лимит потоков, скорость, ETA).
    - [ ] Кликабельные хлебные крошки (Breadcrumbs), история навигации (`<` / `>`), быстрый фильтр и закладки (Favorites).
    - [ ] Интерактивная сортировка по колонкам (Имя, Размер, Права, Дата) и регулировка ширины.
